...
wdd.addEventListener("click", () => {
    renderCourse(courses.filter(course => {
        const wddcourse = course.subject

        return wddcourse === "WDD";
    })
    )
});
...
cse.addEventListener("click", () => {
    renderCourse(courses.filter(course => {
        const csecourse = course.subject

        return csecourse === "CSE";
    })
    )
});
...
function renderCourse(course){
    ...
    for (const info of course) {
        section += `
        <section>
            <h1>${info.completed == true ? "✔" : ""} ${info.subject} ${info.number}</h1>
        </section>
        `
    }
    c.innerHTML = section
